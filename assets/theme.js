...
theme.Search = (function () {
  function init() {
    ...
    $('.searchIco, a[href="#search-modal"]').on('click', function (e) {
      e.preventDefault();
...
theme.collectionView = (function () {
  function collectionView(container) {
    ...
    let isLoadingProductsOnScroll = false;
    // infiniteScroll = function(){
    //   console.log("Function called infiniteScroll");
    //    if (isLoadingProductsOnScroll) return;

    //   	var action = 'scroll load';
    //     $(window).on(action, function(){
    //       console.log("Event called "+ action);
    //       var moreURL = $('.infinitpagin a').attr('href');
    //       console.log("More URL "+moreURL);
    //       if($('.infinitpagin a.infinite').length){
    //         var bottom = $('.infinitpagin').offset();
    //         var docTop = ($(document).scrollTop() + $(window).height() - 50);
    //         console.log("infinitpagin offset ");
    //         console.log(bottom);
    //         console.log("docTop " + docTop);
    //         if( docTop > bottom.top){
    //           $(window).off(action);
    //           isLoadingProductsOnScroll = true;
    //           loadMore(moreURL);
    //         }
    //       }
    //     });
    ...
    infiniteScroll = function () {
      ...
      if (infiniteScrollBound) return; // ✅ prevent rebinding
      ...
      $(window).on('scroll.infiniteScroll', function () {
        ...
        if (docTop > bottom.top) {
          ...
          if (loadedUrls.has(moreURL)) return; // ✅ prevent duplicate URL

          isLoadingProductsOnScroll = true;
          loadedUrls.add(moreURL);
    ...
    loadMore = function (moreURL) {
      console.log("Function called loadMore", moreURL);
      if (moreURL.length) {
        $.ajax({
          type: 'GET',
          dataType: 'html',
          ...
          complete: function (data) {
            ...
            isLoadingProductsOnScroll = false;
            if (theme.mlcurrency) { currenciesChange("sapn.money"); }
            if ($('.infinitpagin a.infinite').length) {
              // infiniteScroll();
            }
